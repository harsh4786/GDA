...
declare_id!("AEpK2fCG2Fkb1EZcYrSgwJhrooNTCMpaKNLmZcp4uu31");
...
pub mod gda {
    ...
    pub fn place_order<'info>(
        ...
        if &ctx.accounts.listing_config.items_sold == &ctx.accounts.listing_config.token_size {
...
pub struct AuctioneerSell<'info>{
    ...
    ///CHECK: This will be checked in cpi
...
pub struct AuctioneerBuy<'info> {
    ...
    #[account(mut, seeds = [PREFIX.as_bytes(), wallet.key().as_ref(), auction_house.key().as_ref(), token_account.key().as_ref(), treasury_mint.key().as_ref(), token_account.mint.as_ref(), token_size.to_le_bytes().as_ref()], seeds::program=auction_house_program, bump = trade_state_bump)]
